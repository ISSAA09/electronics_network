...
class Supplier(models.Model):
    ...
    author = models.ForeignKey(
        User, on_delete=models.CASCADE, verbose_name="Автор поставщика", **NULLABLE
    )
    supply = models.ForeignKey(
        "self", on_delete=models.PROTECT, verbose_name="Поставщик", **NULLABLE
    )
    ...
    network_level = models.IntegerField(
        choices=Level.choices, verbose_name="Уровень структуры"
    )
    debt_to_supplier = models.DecimalField(
        decimal_places=2, max_digits=15, verbose_name="Задолженность", **NULLABLE
    )
