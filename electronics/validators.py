    ...
    def __call__(self, val):
        ...
        if supply:
            if network_level == 0 and supply != self:
                raise ValidationError("A zero-level supplier cannot have another supplier")
            elif network_level == 1 and supply.network_level == 2:
                raise ValidationError("A first-level supplier cannot have a second-level supplier")
